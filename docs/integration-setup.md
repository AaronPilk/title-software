...
The [private Cloudflare pilot](cloudflare-pilot.md) now includes OCR, multiple-company routing and encrypted workspace-token settings. The migrations/API/UI are deployed and the combined five-round suite passed 2,465 cases. Use the private account instructions, choose a personal password and enroll an authenticator. A signed-in staff acceptance walkthrough remains necessary before treating the pilot as ready for real operational work.
...
An earlier Supabase/API/Cloudflare pilot release is deployed. This turn adds OCR, multiple-company Missive routing and encrypted workspace credentials in source; its final release checks, hosted migrations/deployment and credential installation are pending. The complete Missive token passes directory discovery. Company routing/permissions, custom SMTP and a signed-in staff walkthrough still require completion. Actual SoftPro reads/writes, automatic mail classification/outbound messaging, OneDrive automation, DocuSign execution and accounting-provider connections remain unimplemented. No signatures, underwriting actions or payments are sent automatically. See the [requirements implementation guide](REQUIREMENTS_IMPLEMENTATION.md) for current evidence and the historical baseline.
