...
TitleOS is a shared operations workspace for title-company onboarding, policy preparation, company records, and financial review. Both discovery calls support these connected work areas. Tyler’s production workflow is central: incoming attorney documents become reviewed field changes, a preparation package, an underwriter handoff, and separately tracked issuance, delivery, and remittance. The local MVP represents this entire operating cycle with synthetic records and visible review steps.
...
Tyler’s additional 75-minute call makes the immediate sequence concrete: finals first, straightforward commitment revisions second, then initial commitments. He reports roughly 80 pending finals taking 5–10 minutes apiece and approximately 22 active JVs. These are reported estimates, not live dashboard data or promised savings. SoftPro Select and Missive are now confirmed by the recording; exact versions, entitlements and company/profile mappings still need verification. [Timestamped call analysis](../discovery/tyler-call-analysis.md).
...
**Build around the existing SoftPro Select installation.** SoftPro advertises Select extensibility through ProInterface API/SDK, and SoftPro 360 provides integrations with relevant providers. This supports a connector strategy; Tyler’s call identifies Select, while entitlements, approved API operations, company/profile mapping, account configuration, and licensing terms remain to be verified. The first connector should import and reconcile records before writing policy changes. [SoftPro Select](https://www.softprocorp.com/real-estate-software-solutions/softpro-select/), [SoftPro 360](https://www.softprocorp.com/real-estate-software-solutions/softpro-360-data-integration/).

**Use Missive drafts as the first communication handoff.** Tyler welcomes reviewing a prepared reply before sending. Official documentation supports draft creation and attachments; `send: true` and `send_at` are sending controls that the first adapter should omit. The API uses a personal bearer token with that identity’s access, and the draft payload must fit the documented limits. Verify the actual plan and dedicated access before connecting. [Missive REST API](https://missiveapp.com/docs/developers/rest-api), [endpoint reference](https://missiveapp.com/docs/developers/rest-api/endpoints), [detailed adapter plan](tyler-missive-and-records.md).
