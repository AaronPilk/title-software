...
As of September 23, the app has a hosted pilot with Supabase authentication and company-scoped data/storage, separate Agency and Production views, document text/OCR with reviewed field suggestions and source/correction evidence, independent original-file archives, a scoped staff assistant and Missive integration code. These are later additions to the local baseline below. Original documents can be evaluated in the private authorized workspace. Live provider activation must be checked separately: SoftPro integration is deferred at the owner's request and there is no supported live reader/writer yet, and the app does not generate insurer-issued policies, file government applications or move money. Recording a reviewed external outcome is distinct from performing it.
...
| Revisions | Correct-company matching, before/after loan amount, stale checks against both the file's commitment version and the specific loan the request named (resolved from the request itself, never from the current loan count), an explicit loan choice when a file has more than one active loan, local apply that touches only the named loan (the file-level amount mirrors it only while at most one loan is active) and always advances the file's commitment version, reviewed field revisions for lender/seller/legal description/county/attorney/attorney email with the same capture/review/apply shape, revised commitment attachment bound to the post-change version, editable reply and local approval/export; coverage, endorsement and policy-form changes still require separate review |
| Companies and onboarding | Application/signature references, seven evidence stages, multiple required underwriters, formation/operating states, member interests with append-only effective-dated ownership history, authority/renewal records and evidence-based launch; duplicate-company warnings on create (distinctive-name, contact, email and city reasons, never merged or blocked, exact matches acknowledged explicitly); company materials register and reviewed partner publication (see company-materials-publication.md) |
| Documents | Company/order/category/audience filtering, upload/preview, manual classification, source/output version families, downloads and explicit partner visibility; a per-recipient delivery ledger on order documents, bound to one exact version, with retries and a superseded-version guard |
| Financials | Per-policy issued totals, illustrative premium/remittance terms, expenses, remittance review, close revisions with captured rows/ownership resolved from effective-dated records (the position at the end of the reporting month, with the governing record named on the close), books/agreement references, adjustments/reserves, cent-balanced allocations, a manual statement delivery register per published close revision (frozen member/recipient/revision evidence; export and delivery recording kept separate; stale records can only be cancelled; history retained under withdrawn/superseded revisions), and a local CSV upload/preview/column-mapping scaffold with reusable named mapping templates (preview only — no posting to any close, ledger or remittance) |
...
| Tasks and automations | Manual assignment/completion; repeatable intake, onboarding and rejection rules; deduplicated authority-review tasks from recorded dates within 30 days; per-task waiting records and a due-date countdown, with completion blocked while a task is still waiting on someone else |
...
2. **Source interpretation:** OCR proposals tied to original hash/page/excerpt/model, normalized parties/instruments, approved templates, legal-description source binding and complex-revision workflows. The September 23 final-source form now proposes bounded labelled deed/security fields with exact quote/page evidence and a required review step; staff accuracy acceptance remains pending. See [field review](document-field-review.md). Upload limits are 10 files, 25 MB per file and 100 MB per batch.
