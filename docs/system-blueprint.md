...
Updated September 11, 2026; policy-correction workflow, full backup/restore, multi-loan revisions, the rejected-file recovery pipeline, the accounting CSV import scaffold, commitment field revisions and duplicate-company warnings added September 12, 2026 by Claude. This describes the current local implementation. The [54-row transcript matrix](discovery/all-transcript-traceability.md) preserves discovery evidence and pre-expansion gaps; [implementation coverage](implementation-coverage.md) reconciles that baseline with this build.
