...
## Autofill from an application already in Documents

Open the restricted company application and choose **Fill application from this document**. The company’s Application opens with that exact original selected and reading starts automatically. No re-upload or text copying is required. Review the suggestions, apply the reviewed application, then save application details. Reopening the same original starts a fresh review; unsaved work requires a discard decision and pending saves cannot be interrupted.

Application PDFs use visual text positions so flattened answers painted after the questions remain paired with their printed labels. Visible filled form widgets trigger full-page local OCR: canonical PDF field metadata is not trusted over the visible page. Generic document text review and Production extraction retain their existing reading mode.

Choose the date format when the original uses slash dates. This applies to birth dates and dated history rows; a format printed beside a field takes precedence. Written history with partial dates is retained in private notes after review, without inventing exact dates or marking five-year history complete. Blank ownership choices and unreadable answers remain unresolved. Applicant information stays in private intake storage, not shared company profiles. Importing an application does not approve licensing, launch, or issuance.

Regression coverage includes a saved-original entry, three-page flattened overlay PDF, review/save/reopen, explicit dates, history notes, true widget appearance/value mismatch, restricted-role denial, repeated navigation and unsaved/busy guards. Fixtures contain fictional data only.
