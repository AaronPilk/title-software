...
| X05 | Persistent local metadata, IndexedDB assets, source versions, snapshots, local activity, metadata export and full binary backup/restore (bundles every uploaded file's bytes with workspace metadata into one file; reviewed restore with undo) | Authenticated immutable audit, retention, multi-tab/multi-user conflicts and durable server-side recovery |
