...
export function MissiveCredentialSettings({ workspaceId, onChanged }: { workspaceId: string; onChanged: () => void }) {
  ...
  useEffect(() => {
    ...
    backendRequest<MissiveCredentialStatus>("/integrations/missive/credential")
      ...
      .catch(() => { if (!stopped) setError("Connection settings could not be loaded. Refresh to try again."); });
    return () => { stopped = true; };
  }, [workspaceId]);
  async function save(next: string | null) {
    ...
    try {
      ...
      onChanged();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to save connection settings."); }
    finally { setBusy(false); }
  }
  return <section aria-label="Missive account connection">
    <p><strong>{status ? status.configured ? "Missive account connected" : "Connect your Missive account" : "Loading connection…"}</strong>
