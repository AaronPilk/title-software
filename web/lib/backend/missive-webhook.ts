...
export type MissiveWebhookEvent = {
  ...
  payload: { ruleId: string; organizationId: string; teamId: string; companyId: string;
...
export type MissiveWebhookStore = {
  mapping(workspaceId: string): Promise<WebhookMapping | null>;
...
export async function handleMissiveWebhook(req: Request, config: MissiveWebhookConfig, store: MissiveWebhookStore): Promise<Response> {
  ...
  if (!id(conversation.id) || !id(message.id) || !id(record(conversation.organization).id) || !id(record(conversation.team).id) ||
  ...
  try {
    const mapping = await store.mapping(config.workspaceId!);
    if (!mapping || mapping.organizationId !== conversation.organization.id || mapping.teamId !== conversation.team.id)
    ...
    const event: MissiveWebhookEvent = {
      ...
      payload: { ruleId: rule.id, organizationId: mapping.organizationId, teamId: mapping.teamId,
        companyId: mapping.companyId, messageId: message.id, conversationId: conversation.id,
