...
const catalog: CatalogGuide[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Confirm the destination first: Company documents for company records and logos, or Title file documents for a property-specific file. A general upload requires choosing a company; an upload opened inside a company keeps that company selected.",
      "Add the originals, then choose Review documents to check each filename, category and access. Choose Logo / branding for logos; applications use Restricted access. For title-file documents choose the correct existing title file and source type.",
    ...
    note: "The upload form accepts up to 10 files, 25 MB each and 100 MB per batch. Each file can have its own category and access. Check the proposed version before saving a replacement. Authorized originals stay private in the connected workspace; uploads do not email or publish files. Dedicated policy, CPL and correction workflows retain their own output upload controls. Local sample mode is browser-only and should use fictional files.",
  },
  {
    ...
    steps: [
      "Open Companies → select company → Documents for company records and logos. The package reader uses those company documents. Open a file to preview its version or download the original.",
      ...
      "Expand Title-file documents to see originals belonging to a property's title file, with its file number and address. Read those packages in the title file's Final sources.",
      "Use the main Documents page to find accessible files across assigned companies. Filter Company documents or Title-file documents, then use the company and access filters. Filed under shows each destination.",
    ],
