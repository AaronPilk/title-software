...
test('visible onboarding evidence remains read-only for an operations account',async()=>{
  await open('onboarding','role=operations');await page.getByText('Application and approval review',{exact:true}).click();assert.equal(await page.getByLabel('Legal company name',{exact:true}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'Save application',exact:true}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'Record reviewed evidence',exact:true}).isDisabled(),true);assert.deepEqual(await page.evaluate(()=>window.dailyUpdates),[]);
});
test('a company operator with restricted access retains editable application controls',async()=>{
  await open('onboarding','role=onboarding');await page.getByText('Application and approval review',{exact:true}).click();assert.deepEqual(await page.evaluate(()=>window.dailyUpdates),[]);assert.equal(await page.getByLabel('Legal company name',{exact:true}).isEnabled(),true);assert.equal(await page.getByRole('button',{name:'Save application',exact:true}).isEnabled(),true);
});
