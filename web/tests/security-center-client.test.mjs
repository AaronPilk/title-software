...
globalThis.fetch=async(url,init)=>{requests.push({url:String(url),init});return Response.json({items:[],nextCursor:null});};
...
test('security requests pin workspace and account for reads, reviews, and exports',async()=>{
  ...
  const calls=[()=>client.loadSecurityCenter(w,actor),()=>client.listSecurityEvents(w,actor,cursor),()=>client.recordAccessReview({workspaceId:w,snapshotDigest:'a'.repeat(32),note:'Reviewed'},actor),()=>client.exportSecurityEvents(w,actor,cursor)];
