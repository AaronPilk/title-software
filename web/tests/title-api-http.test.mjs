...
async function commandRequest(commands){const result=await handler(new Request('https://backend.example.test/functions/v1/title-api/commands',{method:'POST',headers:{Authorization:'Bearer synthetic-session','Content-Type':'application/json'},body:JSON.stringify({workspaceId:wid,expectedRevision:1,requestId:crypto.randomUUID(),commands})}));return {status:result.status,body:await result.json()};}
for(const commands of [[null],[false],[[]],[{id:crypto.randomUUID(),name:'editDraft',args:[[null]]}]])test(`malformed command never reaches commit ${JSON.stringify(commands)}`,async()=>{const r=await commandRequest(commands);assert.equal(r.status,400);assert(!state.rpcs.some(r=>r.name==='title_commit'));});
for(const table of ['tasks','orders'])test(`connected ${table} manual assignment cannot bypass directory by omitting identity`,async()=>{const r=await commandRequest([{id:crypto.randomUUID(),name:'editDraft',args:[[{table,id:table==='orders'?'order-A':'new-task',insert:table==='tasks',value:table==='orders'?{owner:'Unverified person'}:{companyId:'A',owner:'Unverified person',title:'Task',due:'2026-09-21',done:false,priority:'Normal'}}]]}]);assert.equal(r.status,400);assert.match(r.body.error,/Choose a workspace staff/);assert(!state.rpcs.some(r=>r.name==='title_commit'));});
