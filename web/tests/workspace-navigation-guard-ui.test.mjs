...
before(async()=>{
 ...
 function App(){const detail=useCompanyDetailNavigation();const[identity,setIdentity]=useState({userId:'fictional-owner',email:'owner@example.test',role:'owner',workspaceId:'fixture'});const[allowed,setAllowed]=useState(true);const[company,setCompany]=useState(0);const[accepted,setAccepted]=useState(0);const nav=useWorkspaceView(identity,'Stephenie',true,()=>{setCompany(0);setAccepted(x=>x+1)});return <><button onClick={()=>detail.open('fictional-company','Application','saved-application')}>Fill saved application</button><button onClick={()=>detail.open('fictional-company')}>Reopen same company</button><button onClick={()=>{if(nav.navigate('Companies'))detail.enterAfterNavigation('fictional-company')}}>Search company result</button><output aria-label="Company entry">{JSON.stringify(detail.entry)}</output>{detail.entry.id&&<Editor key={detail.entry.generation} scope="company-detail" label="Entry draft"/>}<output aria-label="Route">{nav.view+'/'+nav.page}</output><output aria-label="Accepted navigations">{accepted}</output><button onClick={()=>nav.navigate('Onboarding')}>Applications</button><button onClick={()=>nav.navigate('Documents')}>Documents</button><button onClick={()=>nav.navigate('Settings')}>Settings</button><button onClick={()=>nav.switchView('production')}>Production</button><button onClick={()=>setIdentity({...identity,userId:'second-fictional-owner'})}>Change account</button><button onClick={()=>setAllowed(false)}>Revoke application access</button><button onClick={()=>setCompany(1)}>Open company</button><button onClick={()=>{if(allowWorkspaceNavigation('company-detail'))setCompany(x=>x+1)}}>Replace company</button>{nav.page==='Onboarding'&&allowed&&<Editor key={identity.userId} scope="workspace" label="Page draft"/>}{company>0&&<Editor key={company} scope="company-detail" label="Company draft"/>}</>}
...
test('repeated same-source entries remount and same-company reopening guards unsaved private work',async()=>{
 await open();const fill=()=>page.getByRole('button',{name:'Fill saved application',exact:true}).click();
 await fill();const first=JSON.parse(await page.getByLabel('Company entry').textContent());await fill();const second=JSON.parse(await page.getByLabel('Company entry').textContent());assert.ok(second.generation>first.generation);assert.equal(second.tab,'Application');assert.equal(second.sourceId,'saved-application');
 await page.getByLabel('Entry draft',{exact:true}).fill('Private unsaved fictional details');let done=decide(false);await page.getByRole('button',{name:'Reopen same company',exact:true}).click();assert.equal(done(),1);assert.equal(await page.getByLabel('Entry draft',{exact:true}).inputValue(),'Private unsaved fictional details');
 done=decide(false);await fill();assert.equal(done(),1);assert.equal(JSON.parse(await page.getByLabel('Company entry').textContent()).generation,second.generation);
 await page.getByRole('button',{name:'Start Entry draft save',exact:true}).click();done=decide(true);await fill();await page.getByRole('button',{name:'Reopen same company',exact:true}).click();assert.equal(done(),0);assert.equal(JSON.parse(await page.getByLabel('Company entry').textContent()).generation,second.generation);
 await page.getByRole('button',{name:'Finish Entry draft save',exact:true}).click();done=decide(true);await page.getByRole('button',{name:'Reopen same company',exact:true}).click();assert.equal(done(),1);assert.equal(JSON.parse(await page.getByLabel('Company entry').textContent()).sourceId,'');
});


test('company search after an accepted workspace transition confirms the departing draft only once',async()=>{
 await open();await page.getByRole('button',{name:'Fill saved application',exact:true}).click();await page.getByLabel('Entry draft',{exact:true}).fill('Unsaved fictional details');const done=decide(true);await page.getByRole('button',{name:'Search company result',exact:true}).click();assert.equal(done(),1);assert.equal(JSON.parse(await page.getByLabel('Company entry').textContent()).tab,'Overview');assert.equal(await page.getByLabel('Entry draft',{exact:true}).inputValue(),'Saved fictional details');
});
